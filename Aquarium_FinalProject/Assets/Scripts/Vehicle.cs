...
public abstract class Vehicle : MonoBehaviour {
    ...
    // makes agent seek center of flock
    public Vector3 Cohesion(List<GameObject> vehicles)
    {
        // neighbors to cohese with
        float neighborDist = 20;
    ...
    // follows a flow field
    public Vector3 FollowField(FlowField field, Vector3 position)
    {
        // get results of GetFlowDirection() in FlowField
        Vector3 flowDirection = field.GetFlowDirection(position);

        // calculate direction at max speed
        Vector3 desiredVelocity = flowDirection.normalized * maxSpeed;
    ...
    // wrap around bounds for flow fields
    public void Wrap()
    {
        if (position.x < 0)
        {
            position.x = Random.Range(theManager.terrain.terrainData.size.x / 2 - 20, theManager.terrain.terrainData.size.x / 2 + 20);
            position.z = 1;
        }
        if (position.z < 0)
        {
            position.z = theManager.terrain.terrainData.size.z - 1;
            position.x = Random.Range(0, theManager.terrain.terrainData.size.x);
        }
        if (position.x > theManager.terrain.terrainData.size.x - 1)
        {
            position.x = Random.Range(theManager.terrain.terrainData.size.z / 2 - 20, theManager.terrain.terrainData.size.z / 2 + 20);
            position.z = 1;
        }
        if (position.z > theManager.terrain.terrainData.size.z - 1)
        {
            position.z = 1;
            position.x = Random.Range(0, theManager.terrain.terrainData.size.x);
        }
    ...
    // !!!specific to final project!!!! Checks for water in front of animal
    public Vector3 CheckLand()
    {
        // height of water and height of terrain in front of vehicle
        float waterHeight = 10.3f;
        float futureHeight = theManager.terrain.SampleHeight(position + (3 * velocity));
        float presentHeight = theManager.terrain.SampleHeight(position);

        Vector3 steeringForce = Vector3.zero;

        // checks if future/present terrain is under water
        if (waterHeight > presentHeight)
        {
            steeringForce = Seek(theManager.centerPos);
        }
        else if (waterHeight > futureHeight)
        {
            // turns vehicle to avoid going into water
            float angle = -1;
    ...
    public Vector3 CheckBounds()
    {
        ...
        if (theManager.maxPos.x < (position.x + (2 * velocity.x)))
        {
            ...
            steeringForce = Seek(new Vector3(theManager.centerPos.x, position.y, position.z));
        }
        ...
        {
            // calculate seek force
            steeringForce = Seek(new Vector3(position.x, position.y, theManager.centerPos.z));
        }
